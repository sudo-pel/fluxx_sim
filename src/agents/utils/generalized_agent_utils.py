...
from src.agents.utils.agent_utils import convert_decision_encoding, populate_card_vector, decision_context_vectors
from src.agents.utils.card_embeddings import get_embedding_table, CARD_EMBED_DIM
from src.game.FluxxEnums import GameState, GameConfig, GamePhaseType, GamePhase
...
def build_action_mask(game_state: GameState, current_phase: GamePhase, player_number: int, game_config: GameConfig) -> np.ndarray:
    if current_phase.type == GamePhaseType.GAME_OVER:
        action_mask = np.zeros(len(game_config.card_list) + 1, dtype=np.int8)
    ...
    cards_in_hand_vector = populate_card_vector(game_config.card_list, game_state.hands[player_number])
    keeper_vectors = [populate_card_vector(game_config.card_list, kl) for kl in game_state.keepers]
    own_keeper_vector = keeper_vectors[player_number]
    other_keeper_vectors = (
        keeper_vectors[: player_number]
        + keeper_vectors[player_number + 1 :]
    )
    rules_vector = populate_card_vector(game_config.card_list, game_state.rules)
    goals_vector = populate_card_vector(game_config.card_list, game_state.goals)
    ...
    if current_phase_type == GamePhaseType.PLAY_CARD_FOR_TURN:
        ...
        action_mask = populate_card_vector(game_config.card_list, [c.name for c in current_phase.latent_space])
        ...
        action_mask[card_to_index[current_phase.labelled_card.name]] = 0
    elif current_phase_type == GamePhaseType.ACTIVATE_FREE_ACTION:
        action_mask = populate_card_vector(game_config.card_list, list(game_state.available_free_actions))
        ...
        valid = [
            c for c in game_state.hands[player_number]
            if CARD_DATA[c]["card_type"] in [t.name for t in current_phase.card_types]
        ]
        action_mask = populate_card_vector(game_config.card_list, valid)
        no_free_action_legal = True
...
def extract_entry(game_state: GameState, player_number: int, game_config: GameConfig) -> BufferEntry:
    """
    ...
    hand = list(game_state.hands[player_number])
    own_keepers = list(game_state.keepers[player_number])
    ...
    hand_size = len(game_state.hands[player_number])
    discard_pile_size = len(game_state.discard_pile)
    own_keepers_in_play_count = len(game_state.keepers[player_number])
    ...
    action_mask = build_action_mask(game_state, current_phase, player_number, game_config)
...
def collate(entries: list[BufferEntry], device: torch.device) -> dict[str, torch.Tensor]:
    N = len(entries)
    ...
    action_mask = np.empty((N, ACTION_DIM), dtype=np.bool_)
